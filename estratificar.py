...
    num (Bool) Boolean variable that indicates whether the point digits should appear in the graph. Default is False, not shows the numbers
    ...
    fig,ax=subplots()
    if num:
    	cor=("blue","white")[poli[0]]
    	va=("top","center")[poli[0]]
        for i in xrange(len(Xn)):
            texto=annotate(str(i+inicio),xy=(Xn[i],Yn[i]),va=va,ha='center',color=cor)
    Xn,Yn=zip(*list(set(zip(Xn,Yn))-set(zip(Xnpri,Ynpri))))
    if poli[0]:	
	    poligonos=[]
	    polipri=[]
	    rpoli=poli[2]
